...
local ol__liuhong = General(extension, "ol__liuhong", "qun", 4)  --这个是十周年先出的叭？-_-||
...
local tuxing = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if event == fk.AreaAborted then
      ...
      return target == player and player:hasSkill(self, true) and player:getMark("@@tuxing_damage") > 0
    end
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
