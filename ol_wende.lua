...
local ol__xianzhu = fk.CreateTriggerSkill{
  ...
  events = {fk.EventPhaseStart},
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self.name) and player.phase == Player.Start and #player:getPile("ol__lisu_zhu") > 0
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    ...
    targets = table.map(table.filter(room:getOtherPlayers(to), function(p)
      return player:inMyAttackRange(p) and not to:isProhibited(p, Fk:cloneCard("slash")) end), function(p) return p.id end)
  ...
  prompt = function()
    if Fk.currentResponsePattern == nil then
      ...
      if Exppattern:Parse(Fk.currentResponsePattern):match(Fk:cloneCard("jink")) and Self:getHandcardNum() == 1 then
        return "#caiwang-jink"
      elseif Exppattern:Parse(Fk.currentResponsePattern):match(Fk:cloneCard("nullification")) and #Self.player_cards[Player.Equip] == 1 then
        return "#caiwang-nullification"
      elseif Exppattern:Parse(Fk.currentResponsePattern):match(Fk:cloneCard("slash")) and #Self.player_cards[Player.Judge] == 1 then
        return "#caiwang-slash"
      end
    end
  end,
...
local chexuan = fk.CreateActiveSkill{
  ...
  can_use = function(self, player)
    return not player:isNude() and #player:getEquipments(Card.SubtypeTreasure) == 0 and #player:getAvailableEquipSlots(Card.SubtypeTreasure) > 0
  end,
...
local chexuan_ts = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(self.name) and #player:getAvailableEquipSlots(Card.SubtypeTreasure) > 0 and #player:getEquipments(Card.SubtypeTreasure) == 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:judge(judge)
    if judge.card.color == Card.Black and not player.dead and #player:getAvailableEquipSlots(Card.SubtypeTreasure) > 0 and #player:getEquipments(Card.SubtypeTreasure) == 0 then
      local ids = {}
      for _, id in ipairs(Fk:getAllCardIds()) do
        local c = Fk:getCardById(id)
        if room:getCardArea(id) == Card.Void then
...
local zhongyun = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(self.name) and player.hp == player:getHandcardNum() and
...
local zhongyun2 = fk.CreateTriggerSkill{
  ...
  events = {fk.AfterCardsMove},
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(self.name) and player.hp == player:getHandcardNum() and player:usedSkillTimes(self.name) == 0 then
      for _, move in ipairs(data) do
...
local canmou = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(self.name) and data.targetGroup and data.firstTarget and data.card:isCommonTrick() and
...
local congjianx = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(self.name) and target ~= player and data.card:isCommonTrick() and
...
local maihuo = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self.name) and data.card.trueName == "slash" and #TargetGroup:getRealTargets(data.tos) == 1
