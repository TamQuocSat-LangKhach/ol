...
local qigong = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self.name) and data.card.trueName == "slash" and #data.tos == 1
  end,
  on_cost = function(self, event, target, player, data)
    local room = player.room
    local targets = table.map(room:getOtherPlayers(room:getPlayerById(data.to)), function(p) return p.id end)
...
local liehou = fk.CreateActiveSkill{
  ...
  on_use = function(self, room, effect)
    ...
    local targets = table.map(table.filter(room:getOtherPlayers(player), function(p)
      return player:inMyAttackRange(p) and p ~= target end), function(p) return p.id end)
...
local saodi = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(self.name) and (data.card.trueName == "slash" or data.card:isCommonTrick()) and
...
local zhuitao = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if #targets == 0 then return end
    local to = player.room:askForChoosePlayers(player, table.map(targets, function(p) return p.id end), 1, 1, "#zhuitao-choose", self.name, true)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local yuanchou = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self.name) and data.card and data.card.trueName == "slash" and data.card.color == Card.Black
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
...
local cihuang = fk.CreateTriggerSkill{
  ...
  events = {fk.CardEffectCancelledOut},
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(self.name) and data.from and player.room:getPlayerById(data.from).phase ~= Player.NotActive and
