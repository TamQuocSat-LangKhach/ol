...
local benyu = fk.CreateTriggerSkill{
  ...
  events ={fk.Damaged},
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self) and data.from and not data.from.dead and player:getHandcardNum() ~= data.from:getHandcardNum()
  end,
  on_cost = function(self, event, target, player, data)
    local room = player.room
...
local bushi = fk.CreateTriggerSkill{
  ...
  events = {fk.Damaged},
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(self) and (target == player or data.from == player) and #player:getPile("zhanglu_mi") > 0 and not (data.from.dead or data.to.dead)
  end,
...
local chouce = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    elseif judge.card.color == Card.Black then
      local targets = table.map(table.filter(room:getAlivePlayers(), function(p) return not p:isAllNude() end), Util.IdMapper)
...
local lianpian = fk.CreateTriggerSkill{
  ...
  events = {fk.TargetSpecified},
  can_trigger = function(self, event, target, player, data)
    if not player:hasSkill(self) or player ~= target or player.phase ~= Player.Play or player:usedSkillTimes(self.name) > 2 or not data.firstTarget or not data.tos then return false end
    local room = player.room
    local tos
...
local qingzhong = fk.CreateTriggerSkill{
  ...
  can_refresh = function(self, event, target, player, data)
    return target == player and player:hasSkill(self) and player:usedSkillTimes(self.name, Player.HistoryPhase) > 0 and not player:isKongcheng()
  end,
  on_refresh = function(self, event, target, player, data)
    local room = player.room
...
local neifa = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    local room = player.room
    local targets = table.map(table.filter(room:getAlivePlayers(), function(p)
      return #p:getCardIds{Player.Equip, Player.Judge} > 0 end), Util.IdMapper)
    if #targets == 0 then
      player:drawCards(2, self.name)
    else
...
local bizheng = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local to = player.room:askForChoosePlayers(player, table.map(player.room:getOtherPlayers(player), Util.IdMapper), 1, 1, "#bizheng-choose", self.name, true)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local yidian = fk.CreateTriggerSkill{
  ...
  events = {fk.TargetSpecifying},
  can_trigger = function(self, event, target, player, data)
    if target == player and player:hasSkill(self) and data.targetGroup and data.firstTarget and
