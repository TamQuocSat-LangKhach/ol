...
local ol_ex__leiji = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    elseif event == fk.FinishJudge then
      local x = 2
      if data.card.suit == Card.Club and player:isWounded() then
        x = 1
        if player:isWounded() then
          room:recover({
            who = player,
...
local ol_ex__baonve_delay = fk.CreateTriggerSkill{
  ...
  mute = true,
  can_trigger = function(self, event, target, player, data)
    return target == player and data.card.suit == Card.Spade and player.room:getCardArea(data.card.id) == Card.Processing
  end,
  on_cost = function() return true end,
  on_use = function(self, event, target, player, data)
    local room = player.room
...
local ol_ex__wansha_invalidity = fk.CreateInvaliditySkill {
  ...
  invalidity_func = function(self, from, skill)
    if skill.frequency ~= Skill.Compulsory and skill.frequency ~= Skill.Wake and not skill.attached_equip then
      return table.find(Fk:currentRoom().alive_players, function(p)
        return p.dying
...
local ol_ex__luanwu = fk.CreateActiveSkill{
  ...
  on_use = function(self, room, effect)
    ...
    for _, target in ipairs(targets) do
      ...
      local use = room:askForUseCard(target, "slash", "slash", "#ol_ex__luanwu-use", true, {must_targets = luanwu_targets})
      if use then
        room:useCard(use)
      else
...
Fk:loadTranslationTable{
  ["ol_ex__jiaxu"] = "界贾诩",
  ["ol_ex__wansha"] = "完杀",
  [":ol_ex__wansha"] = "①除进行濒死流程的角色以外的其他角色于你的回合内不能使用【桃】。②在一名角色于你的回合内进行的濒死流程中，除其以外的其他角色的不带“锁定技”标签的技能无效。",
  ["ol_ex__luanwu"] = "乱武",
  ...
  ["#ol_ex__luanwu-use"] = "乱武：你需要对距离最近的角色使用一张【杀】，否则失去1点体力",
