...
this:addEffect("viewas", {
  ...
  before_use = function(self, player, use)
    ...
    room:moveCardTo(cards, Card.Processing, nil, fk.ReasonPut, this.name, "", false, player.id)
    ...
    if #players > 0 then
      local questioners = {}
      local result = U.askForJointChoice(players, {"noquestion", "question"}, this.name,
      ...
      if #questioners > 0 then
        ...
        if use.card.name == Fk:getCardById(card_id).name then
          ...
          for _, p in ipairs(questioners) do
            if not p.dead then
              if #room:askToDiscard(p, { min_num = 1, max_num = 1, include_equip = true, skill_name = this.name, cancelable = true, pattern = ".", prompt = "#ol_ex__guhuo-discard"}) == 0 then
                room:loseHp(p, 1, this.name)
              end
            end
    ...
    if canuse then
      ...
      room:moveCardTo(card_id, Card.DiscardPile, nil, fk.ReasonPutIntoDiscardPile, this.name)
      return ""
    end
  end,
  enabled_at_play = function(self, player)
    return not player:isKongcheng() and player:usedSkillTimes(this.name, Player.HistoryTurn) == 0
  end,
