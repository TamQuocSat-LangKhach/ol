...
this:addEffect(fk.AfterCardsMove, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(this.name) and player:usedSkillTimes(this.name, Player.HistoryPhase) < 1 then
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if #targets == 1 then
      if room:askToSkillInvoke(player, { skill_name = this.name, prompt = "#ol_ex__guzheng-invoke::"..targets[1].id}) then
        self.cost_data = {tos = targets, cards = card_pack[1]}
        return true
      end
    elseif #targets > 1 then
      local tos = room:askToChoosePlayers(player, { targets = targets, min_num = 1, max_num = 1, prompt = "#ol_ex__guzheng-choose", skill_name = this.name})
      if #tos > 0 then
  ...
  on_use = function(self, event, target, player, data)
    ...
    if #cards > 1 then
      to_return, choice = U.askforChooseCardsAndChoice(player, cards, {"guzheng_yes", "guzheng_no"}, this.name,
      "#guzheng-title::" .. to.id)
    end
    local moveInfos = {}
